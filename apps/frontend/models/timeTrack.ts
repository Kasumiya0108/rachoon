...
class TimeTrack implements TimeTrackType {
  ...
  public recalc() {
    if (this.duration.includes(":")) {
      this.data.minutes =
        Number(this.duration.split(":")[0]) * 60 +
        Number(this.duration.split(":")[1]);
    } else {
  ...
  public net() {
    return (
      (((this.data.minutes / 60) *
        (this.project.data.rate || this.project.client.data.conditions.rate)) /
        100) *
        100 -
      (this.project.data.discount || 0)
    );
  }
